...
## from validations()
<input v-model.lazy='$v.form.login.$model'>
...
<div v-for="(item, index) in $v.forms.$each.$iter">
  <input v-model="item.val.$model"                    // берем из validations().
         :class="{error: !item.val.integer && !item.val.between,
...
validations: {
  forms: {
    ...
    $each: {
      val: {                  //какие поля у членов массива будем валидировать.
        required,
...
<div v-for="(field, key, ind) of forms">
        <div :class="{
               'error': $v.forms[key].val.$error || forms[key].isDirty,
               'valid': !$v.forms[key].val.$invalid && forms[key].val
             }"
        >
          {{field.id}}
        </div>
...
<div :class="{error: $v.name.$error  || fieldNoDirty,
              ...
              valid: !$v.name.$invalid && item.val.$model         //v-for of validations().
     }"
> 
</div>
